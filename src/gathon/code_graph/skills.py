...
def generate_cursor_hooks_config() -> dict[str, Any]:
    ...
    return {
        ...
        "hooks": {
            "afterFileEdit": [
                {
                    "command": f"{hooks_dir}/gathon-update.sh",
                    "timeout": 5,
                },
            ],
            "sessionStart": [
                {
                    "command": f"{hooks_dir}/gathon-session-start.sh",
                    "timeout": 5,
                },
            ],
            "beforeShellExecution": [
                {
                    ...
                    "command": f"{hooks_dir}/gathon-pre-commit.sh",
                    "timeout": 10,
                },
            ],
        },
...
def _cursor_hook_scripts() -> dict[str, str]:
    ...
    - gathon-update.sh: runs ``code-review-graph update --skip-flows`` after file edits
    - gathon-session-start.sh: runs ``code-review-graph status`` on session start
    - gathon-pre-commit.sh: runs ``code-review-graph detect-changes --brief`` before
      git commit commands
...
"""
    ...
    return {
        "gathon-update.sh": update_script,
        "gathon-session-start.sh": session_start_script,
        "gathon-pre-commit.sh": pre_commit_script,
    }
...
def install_opencode_plugin() -> Path:
    ...
    Writes ``~/.config/opencode/plugins/gathon-plugin.ts``.  Creates the
    ...
    plugin_path = plugins_dir / "gathon-plugin.ts"
