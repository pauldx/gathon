...
Manages a registry of multiple repositories at ``~/.gathon/registry.json``
...
_REGISTRY_DIR = Path.home() / ".gathon"
_REGISTRY_PATH = _REGISTRY_DIR / "registry.json"


class Registry:
    """Manages a JSON-based registry of gathon repositories.

    Each entry stores the repo path and an optional alias.
    The registry lives at ``~/.gathon/registry.json``.
    """
    ...
    def register(self, path: str, alias: str | None = None) -> dict[str, str]:
        ...
        Validates that the path contains a ``.git`` or ``.gathon``
        directory.
        ...
        if not (resolved / ".git").exists() and not (resolved / ".gathon").exists():
            raise ValueError(
                f"Path does not look like a repository "
                f"(no .git or .gathon): {resolved}"
            )
