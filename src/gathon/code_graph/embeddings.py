...
class LocalEmbeddingProvider(EmbeddingProvider):
    def __init__(self, model_name: str | None = None) -> None:
        self._model_name = model_name or os.environ.get(
            "GATHON_EMBEDDING_MODEL", LOCAL_DEFAULT_MODEL
        )
...
class OpenAIEmbeddingProvider(EmbeddingProvider):
    ...
    def name(self) -> str:
        ...
        # table so switching GATHON_OPENAI_BASE_URL triggers a safe re-embed.
...
def _warn_cloud_egress(provider_name: str) -> None:
    ...
    The warning is suppressed when ``GATHON_ACCEPT_CLOUD_EMBEDDINGS=1`` is
    ...
    """
    if os.environ.get("GATHON_ACCEPT_CLOUD_EMBEDDINGS", "").strip() == "1":
        return
    print(
        ...
        "GATHON_ACCEPT_CLOUD_EMBEDDINGS=1 in your environment.\n"
...
def get_provider(
    ...
    Args:
        provider: Provider name. One of "local", "google", "minimax", "openai",
                  ...
                  OpenAI requires GATHON_OPENAI_API_KEY + GATHON_OPENAI_BASE_URL +
                  GATHON_OPENAI_MODEL env vars (or the ``model`` arg). The egress
                  warning is skipped when the base URL points to localhost.
                  Cloud providers emit a one-time stderr warning before use
                  unless ``GATHON_ACCEPT_CLOUD_EMBEDDINGS=1`` is set. See: #174
        model: Model name/path to use. For local provider this is any
               sentence-transformers compatible model. Falls back to
               GATHON_EMBEDDING_MODEL env var, then to all-MiniLM-L6-v2.
               For Google provider this is a Gemini model ID.
               For OpenAI provider this overrides GATHON_OPENAI_MODEL.
    """
    if provider == "openai":
        api_key = os.environ.get("GATHON_OPENAI_API_KEY")
        base_url = os.environ.get("GATHON_OPENAI_BASE_URL")
        resolved_model = model or os.environ.get("GATHON_OPENAI_MODEL")
        if not api_key or not base_url or not resolved_model:
            missing = [
                name for name, val in [
                    ("GATHON_OPENAI_API_KEY", api_key),
                    ("GATHON_OPENAI_BASE_URL", base_url),
                    ("GATHON_OPENAI_MODEL", resolved_model),
                ] if not val
            ]
            raise ValueError(
        ...
        dim_env = os.environ.get("GATHON_OPENAI_DIMENSION")
        dimension = int(dim_env) if dim_env else None
        batch_env = os.environ.get("GATHON_OPENAI_BATCH_SIZE")
