...
_MAX_PARSE_WORKERS = int(os.environ.get("GATHON_PARSE_WORKERS", str(min(os.cpu_count() or 4, 8))))
...
def find_project_root(
    ...
    1. ``GATHON_REPO_ROOT`` environment variable — explicit override for
    ...
    """
    env_override = os.environ.get("GATHON_REPO_ROOT", "").strip()
    if env_override:
...
def get_data_dir(repo_root: Path) -> Path:
    ...
    ``GATHON_DATA_DIR`` environment variable is set, it is used verbatim
    ...
    """
    env_override = os.environ.get("GATHON_DATA_DIR", "").strip()
    if env_override:
...
def get_db_path(repo_root: Path) -> Path:
    ...
    Respects ``GATHON_DATA_DIR`` (see :func:`get_data_dir`). Migrates a
    legacy top-level ``.code-review-graph.db`` file into the new
    directory when it exists (WAL/SHM side-files are discarded).
    """
    gathon_dir = get_data_dir(repo_root)
    new_db = gathon_dir / "graph.db"

    # Migrate legacy database if present (only meaningful when the
    # legacy file sits at the repo root — if GATHON_DATA_DIR is set we
...
def ensure_repo_gitignore_excludes_gathon(repo_root: Path) -> str:
...
_GIT_TIMEOUT = int(os.environ.get("GATHON_GIT_TIMEOUT", "30"))  # seconds, configurable
...
_RECURSE_SUBMODULES = os.environ.get("GATHON_RECURSE_SUBMODULES", "").lower() in ("1", "true", "yes")
...
def get_all_tracked_files(
    ...
    Args:
        ...
        recurse_submodules: If True, pass ``--recurse-submodules`` to
            ...
            ``GATHON_RECURSE_SUBMODULES`` environment variable.
...
def collect_all_files(
    ...
    Args:
        ...
        recurse_submodules: If True, include files from git submodules.
            When *None*, falls back to ``GATHON_RECURSE_SUBMODULES`` env var.
    """
...
_MAX_DEPENDENT_HOPS = int(os.environ.get("GATHON_DEPENDENT_HOPS", "2"))
...
def full_build(
    ...
    use_serial = os.environ.get("GATHON_SERIAL_PARSE", "") == "1"
